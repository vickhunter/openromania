...
(function($) {
    ...
    $("#requestForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            ...
            rformError();
            rsubmitMSG(false, "Bisogna completare tutti i campi per inviare la richiesta!");
        } else {
    ...
    $("#contactForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            // handle the invalid form...
            cformError();
            csubmitMSG(false, "Bisogna completare tutti i campi per inviare la richiesta!");
        } else {
            // everything looks good!
            event.preventDefault();
            csubmitForm();
    ...
    $("#privacyForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            // handle the invalid form...
            pformError();
            psubmitMSG(false, "Bisogna completare tutti i campi per inviare la richiesta");
        } else {
            // everything looks good!
            event.preventDefault();
            psubmitForm();
    ...
    function pformSuccess() {
        $("#privacyForm")[0].reset();
        psubmitMSG(true, "Richiesta inviata con successo!");
        $("input").removeClass('notEmpty'); // resets the field label after submission
    }
    ...
    $('body').prepend('<a href="body" class="back-to-top page-scroll">Ritorna su</a>');
